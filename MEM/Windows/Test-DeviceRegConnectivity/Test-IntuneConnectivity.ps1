...
.SYNOPSIS
    Test-IntuneConnectivity V3.1 PowerShell script.
...
#>
# Intune Endpoints
$endpoints =  New-Object -TypeName System.Collections.ArrayList
$endpoints.AddRange(@(
    "https://login.microsoftonline.com",
    "https://device.login.microsoftonline.com",
    "https://enterpriseregistration.windows.net"
...
Function Test-DevRegConnectivity{
    ...
    if ($global:TestFailed){
        Write-Host ''
        Write-Host ''
        Write-Host "Test failed: device is not able to communicate with Intune endpoints under system account" -ForegroundColor red
        Write-Log -Message "Test failed: device is not able to communicate with Intune endpoints under system account" -Level ERROR
        Write-Host ''
        ...
        Write-Host ''
        Write-Host "Test passed: Device is able to communicate with Intune endpoints successfully under system context" -ForegroundColor Green
