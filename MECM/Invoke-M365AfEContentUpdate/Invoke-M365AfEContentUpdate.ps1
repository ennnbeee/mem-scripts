...
.NOTES
    FileName:    Invoke-M365AfEContentUpdate.ps1
    Author:      Nick Benton - Phoenix Software
    Contact:     nick-benton@phoenixs.co.uk
    Created:     2019-10-22
    Updated:     2021-08-13
    ...
    1.0.5 - (2021-08-13) Updated to detect multiple M365 AfE Applications and Update Content for Multiple Deployment types
#>
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        try {
                                            ...
                                            # Get all Office Applications - added due to multiple M365 AfE applications - 20210813 Nick Benton Phoenix Software
                                            $OfficeApplications = Get-CMApplication -Fast | Where-Object {$_.LocalizedDisplayName -like $("$OfficeApplicationName*")} | Select-object -ExpandProperty LocalizedDisplayName
                                            foreach($OfficeApplication in $OfficeApplications){
                                                Write-Verbose -Message "Attempting to retrieve Office 365 ProPlus application deployment type for application: $($OfficeApplication)"
                                            
                                                $OfficeDeploymentType = Get-CMDeploymentType -ApplicationName $OfficeApplication -ErrorAction Stop -Verbose:$false
                                            ...
                                            try {
                                                ...
                                                # Update Distribution Points - Updated to loop through deployments - 20210810 Nick Benton Phoenix Software
                                                Write-Verbose -Message “Attempting to update Distribution Points for application: $($OfficeApplication)”
                                                foreach ($deployment in $OfficeDeploymentType.LocalizedDisplayName){
                                                Update-CMDistributionPoint -ApplicationName $OfficeApplication -DeploymentTypeName $deployment -ErrorAction Stop -Verbose:$false
                                                }
